"""
Energy-weighted mean wavenumber, k_ave = int(k*E(k) dk) / int(E(k) dk),
for each of the 9 (field, direction) cases across the chi scan.

Analog of kmax_vs_chi.py, but instead of locating the peak of the
compensated spectrum k*E(k), this integrates the raw spectrum E(k) over k
(trapezoidal rule) to get an energy-weighted average k -- a smoother,
whole-spectrum diagnostic rather than a single-bin peak location.

As in the other averaged-spectra scripts, each run's spectrum is first
averaged over its last N_LAST timesteps, and k_ave is then computed from
that time-averaged spectrum. The k=0 (DC / mean-field) mode is excluded
from the integral, consistent with kmax_vs_chi.py's peak search -- k=0
represents the background/mean state, not a turbulent fluctuation, and
its (possibly large) energy would otherwise bias k_ave down without
reflecting any real spectral scale.
"""
import os
import re
...
PLOT_OUTPATH = os.path.join(SCRIPT_DIR, "kave_vs_chi.png")
TABLE_OUTPATH = os.path.join(SCRIPT_DIR, "kave_vs_chi_table.txt")
...
def compute_kave_table(run_dir, n_last=N_LAST):
    ...
    for field in fields:
        for direction in directions:
            ...
            numerator = np.trapz(kk * ee, kk)
            denominator = np.trapz(ee, kk)
...
chi_values = []
kave_by_key = {f"{field}_{direction}": [] for field in fields for direction in directions}

for name in run_dirs:
    run_dir = os.path.join(SCAN_DIR, name)
    chi = parse_chi(name)
    print(f"processing {name} (chi={chi:.6g}) ...")
    table = compute_kave_table(run_dir)
    chi_values.append(chi)
    for key, value in table.items():
        kave_by_key[key].append(value)
...
for row, field in enumerate(fields):
    for col, direction in enumerate(directions):
        ...
        values = np.array(kave_by_key[key])
        ...
        if col == 0:
            ax.set_ylabel(f"$k_{{ave}}$ ({field})")
        if row == len(fields) - 1:
            ax.set_xlabel(r"$\chi$")
        ax.legend(fontsize=6)

fig.suptitle(f"Energy-weighted mean wavenumber vs $\\chi$ (spectra averaged over last {N_LAST} timesteps)")
fig.tight_layout()
fig.savefig(PLOT_OUTPATH, dpi=150, bbox_inches="tight")
print(f"saved {PLOT_OUTPATH}")

with open(TABLE_OUTPATH, "w") as fh:
    fh.write("chi\t" + "\t".join(kave_by_key.keys()) + "\n")
    for i in order:
        row_vals = "\t".join(f"{kave_by_key[key][i]:.6g}" for key in kave_by_key)
