"""
Normalized spectral width (sigma_k / k_ave), averaged over the last N_LAST
timesteps, vs chi.

Reuses spectral_width_vs_time.py's per-timestep spectral_width_time_series()
(no spectral smoothing -- raw sigma_k/k_ave at each saved snapshot), then
simply averages the trailing N_LAST values of that already-computed time
series for each chi, rather than re-averaging the raw spectra first.
"""
import os
import re
...
PLOT_OUTPATH = os.path.join(SCRIPT_DIR, "spectral_width_vs_chi.png")
TABLE_OUTPATH = os.path.join(SCRIPT_DIR, "spectral_width_vs_chi_table.txt")
...
def spectral_width_time_series(run_dir):
    """Returns (time, {field_direction: sigma_k(t)/k_ave(t)}) for one run."""
    ...
    for field in fields:
        for direction in directions:
            ...
            total_energy = np.trapz(ee, kk, axis=1)
            k_ave = np.trapz(kk * ee, kk, axis=1) / total_energy
            variance = np.trapz((kk - k_ave[:, None]) ** 2 * ee, kk, axis=1) / total_energy
            sigma_k = np.sqrt(variance)

            result[f"{field}_{direction}"] = sigma_k / k_ave
    return time, result
...
chi_values = []
avg_width_by_key = {f"{field}_{direction}": [] for field in fields for direction in directions}

for name in run_dirs:
    run_dir = os.path.join(SCAN_DIR, name)
    chi = parse_chi(name)
    print(f"processing {name} (chi={chi:.6g}) ...")
    time, width_dict = spectral_width_time_series(run_dir)
    if len(time) < N_LAST:
        raise ValueError(f"{run_dir}: only {len(time)} timesteps available, requested N_LAST={N_LAST}")
    chi_values.append(chi)
    for key, series in width_dict.items():
        avg_width_by_key[key].append(np.mean(series[-N_LAST:]))
...
for row, field in enumerate(fields):
    for col, direction in enumerate(directions):
        ...
        values = np.array(avg_width_by_key[key])
        ...
        if col == 0:
            ax.set_ylabel(f"$\\sigma_k / k_{{ave}}$ ({field})")
        if row == len(fields) - 1:
            ax.set_xlabel(r"$\chi$")

fig.suptitle(f"Normalized spectral width vs $\\chi$ (averaged over last {N_LAST} timesteps)")
fig.tight_layout()
fig.savefig(PLOT_OUTPATH, dpi=150, bbox_inches="tight")
print(f"saved {PLOT_OUTPATH}")

with open(TABLE_OUTPATH, "w") as fh:
    fh.write("chi\t" + "\t".join(avg_width_by_key.keys()) + "\n")
    for i in order:
        row_vals = "\t".join(f"{avg_width_by_key[key][i]:.6g}" for key in avg_width_by_key)
